...
public class ByteUtils {
    ...
    public static String byteArrToHexString(byte[] b, boolean isSplit){
        String result="";
        for (int i=0; i < b.length; i++) {
            if(isSplit) {
                result += Integer.toString((b[i] & 0xff) + 0x100, 16).substring(1) + " ";
    ...
    public static short[] byteArrayToShortArray(byte[] src) {
        ...
        short[] shortSamples = new short[len / 2];
        //取出shortBuffer中的short数组
        shortBuffer.get(shortSamples, 0, shortSamples.length);
        return shortSamples;
    }

    public static byte[] shortArrayToByteArray(short[] src) {
        ByteBuffer byteBuffer = ByteBuffer.allocate(src.length * 2);
