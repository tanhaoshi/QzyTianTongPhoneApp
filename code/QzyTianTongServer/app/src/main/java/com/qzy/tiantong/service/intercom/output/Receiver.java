...
public class Receiver extends JobHandler {
    ...
    public void run() {
        // 设置接收缓冲段
        byte[] receivedData = new byte[160 * 6 * 2 + 1];
        while (true) {
    ...
    private void handleAudioData(DatagramPacket packet) {
       ...
       // LogUtils.e("reece pcm index = " + ByteUtils.byteToInt(packet.getData()[0]));
        AudioData audioData = new AudioData(Arrays.copyOfRange(packet.getData(),1,packet.getData().length));
